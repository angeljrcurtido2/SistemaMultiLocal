...
// ─────────────────────────────────────────────────────────────────────────────
// Vista previa del comprobante de venta antes de imprimir — mismo patrón que
// ComprobantePreviaWindow (Cobros), pero para DatosTicketVenta en vez de
// DatosTicketCobro. Se creó aparte en vez de generalizar la de Cobros para no
// arriesgar romper ese flujo ya probado con un cambio de tipo genérico.
// ─────────────────────────────────────────────────────────────────────────────
public class ComprobanteVentaPreviaWindow : Window
{
    private readonly DatosTicketVenta _datos;
    ...
    public ComprobanteVentaPreviaWindow(DatosTicketVenta datos, byte formato)
    {
        _datos   = datos;
        _formato = formato;

        var fmtLabel = formato switch { 1 => "Ticket 58 mm", 2 => "Ticket 40 mm", 3 => "Ticket 80 mm", _ => "A4 Normal" };
        var tipoLabel = datos.EsContado ? "Venta al Contado" : "Venta a Crédito";
        Title = $"Vista previa — Comprobante de {tipoLabel} ({fmtLabel})";
    ...
    private void BuildUI()
    {
        ...
        var bar = new Border { Background = B("#1A237E"), Padding = new Thickness(14, 10, 14, 10) };
        ...
        btnImp.Click += async (_, _) => { await TicketPrinter.ImprimirVentaAsync(_datos); Close(); };
    ...
    private void Renderizar()
    {
        ...
        switch (_formato)
        {
            case 1:
            case 2:
            case 3: TicketPrinter.DibujarVentaTicketPublic(gfx, _datos, AnchoMmDe(_formato)); break;
            default: TicketPrinter.DibujarVentaA4Public(gfx, _datos); break;
        }
    ...
    {
        var doc = new System.Drawing.Printing.PrintDocument { DocumentName = "Comprobante_Venta" };
        doc.PrintPage += (_, e) => {
            e.HasMorePages = false;
            switch (_formato) {
                case 1:
                case 2:
                case 3: TicketPrinter.DibujarVentaTicketPublic(e.Graphics!, _datos, AnchoMmDe(_formato)); break;
                default: TicketPrinter.DibujarVentaA4Public(e.Graphics!, _datos); break;
            }
        };
