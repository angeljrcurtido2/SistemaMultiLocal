...
public static class ArticulosListadoTablaImpresora
{
    public static async Task ImprimirAsync(ArticulosListadoTablaPagina p, Window? owner = null)
    {
        var doc = new PrintDocument { DocumentName = "Listado de Artículos" };
        // "Todos los locales" tiene muchas columnas (una por local) y necesita apaisada;
        // "Específico" son solo 7 columnas fijas y entra cómodo en vertical — pedido
        // explícito del usuario, no girar la hoja cuando no hace falta.
        doc.DefaultPageSettings.Landscape = p.TodosLosLocales;
        ...
        if (esPdf)
        {
            ...
            var sfd = new Microsoft.Win32.SaveFileDialog
            {
                ...
                DefaultExt       = "pdf",
                FileName         = $"ListadoArticulos_TodosLosLocales_{DateTime.Now:yyyyMMdd_HHmm}",
        ...
        await Task.Run(() =>
        {
            ...
            doc.PrintPage += (_, e) =>
            {
                ...
                e.HasMorePages = ArticulosListadoTablaDibujador.DibujarPagina(g, p, logo, ref filaOffset, pageNum, totalPages);
            };
    ...
    public static void Imprimir(ArticulosListadoTablaPagina p, Window? owner = null)
